...
int op_set_decode_channel_mapping(OggOpusFile *_of,
 const unsigned char *_mapping,int _channel_count){
  ...
  if(_channel_count<=0&&_of->output_channel_count<=0)return 0;
  ready_state=_of->ready_state;
  if(ready_state>=OP_STREAMSET&&_channel_count>0){
    head=&_of->links[_of->seekable?_of->cur_link:0].head;
    if(head->mapping_family==3)return OP_EIMPL;
    for(ci=0;ci<_channel_count;ci++){
      if(_mapping[ci]>=head->channel_count)return OP_EINVAL;
    }
  }
...
int op_clear_decode_channel_mapping(OggOpusFile *_of){
  return op_set_decode_channel_mapping(_of,NULL,0);
}
