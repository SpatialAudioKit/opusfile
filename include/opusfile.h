...
/**Sets a channel subset/remapping for decoded output.
   ...
   \retval 0          Success.
   \retval #OP_EFAULT An internal memory allocation failed.
   \retval #OP_EIMPL  The current link uses channel mapping family 3, which
                        does not support channel subsets.
   \retval #OP_EINVAL The mapping count, pointer, or a source channel index was
                        invalid.*/
...
/**Clears any channel subset/remapping set by op_set_decode_channel_mapping().
   This is a no-op if no mapping is active.
   \retval 0          Success, or no mapping was active.
   \retval #OP_EFAULT An internal memory allocation failed.
   \retval #OP_EIMPL  The decoder for the current link is not implemented.*/
OP_WARN_UNUSED_RESULT int op_clear_decode_channel_mapping(OggOpusFile *_of)
 OP_ARG_NONNULL(1);
